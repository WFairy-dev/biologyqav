...
from chat.server.file_rag.text_splitter import (
    zh_title_enhance as func_zh_title_enhance,
)
from chat.server.utils import run_in_process_pool, run_in_thread_pool
    ...
    try:
        if (
            ...
            try:  # 优先使用用户自定义的text_splitter
                text_splitter_module = importlib.import_module("server.file_rag.text_splitter")
                TextSplitter = getattr(text_splitter_module, splitter_name)
                print("++++++++++text_splitter_module:",TextSplitter)
            except:  # 否则使用langchain的text_splitter
                text_splitter_module = importlib.import_module(
                    "langchain.text_splitter"
                )
                TextSplitter = getattr(text_splitter_module, splitter_name)
                print("分词器：",text_splitter_module, splitter_name)
            if (
        ...
        TextSplitter = getattr(text_splitter_module, "ChineseSentenceTextSplitter")
...
class KnowledgeFile:
    ...
    def docs2texts(
        ...
        if self.ext not in [".csv"]:
            ...
            print(1233333333,self.text_splitter_name)
