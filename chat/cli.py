...
from init_database import main as kb_main, create_tables, folder2db
...
@click.group(help="Biology Chat 命令行工具")
def main():
    ...
...
@click.option("-x", "--xinference-endpoint", "xf_endpoint",
              help="指定Xinference API 服务地址。默认为 http://127.0.0.1:9998/v1")
...
def init(
    ...
    print(f"bs.PACKAGE_ROOT: {bs.PACKAGE_ROOT}")
    ...
    if recreate_kb:
        ...
        logger.success("<green>所有初始化已完成，执行 python startup.py -a 启动服务。</green>")
    else:
        logger.success("执行 python init_database.py -r 初始化知识库，然后 python startup.py -a 启动服务。")
